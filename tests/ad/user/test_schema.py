...
@pytest.fixture()
def users_schema_for_login():
    return {
        'authToken': 'something'
    }


def test_users_schema_for_login(users_schema_for_login):
    '''
    Test the users schema for login
    '''
    test_resp = {
        'authToken': 'something'
    }
    ...
    schema = UserSchema()
    assert test_resp['authToken'] == schema.dump(schema.load(users_schema_for_login))['authToken']
    with pytest.raises(ValidationError):
        users_schema_for_login['some_val'] = 'something'
        schema.load(users_schema_for_login)
